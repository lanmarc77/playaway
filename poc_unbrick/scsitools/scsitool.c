...
static int do_info(void)
{
    ...
    struct stmp_logical_media_table_t *tabl;
    ret = stmp_get_logical_media_table(g_dev_fd, &tabl);
    if(!ret)
    {
        cprintf(BLUE, "Logical Media Table\n");
        for(int i = 0; i < tabl->header.count; i++)
        {
            cprintf(RED, "  Drive ");
            cprintf_field("No: ", "%2x", tabl->entry[i].drive_no);
            cprintf_field(" Type: ", "%#x ", tabl->entry[i].type);
            cprintf(RED, "(%s)", stmp_get_logical_drive_type_string(tabl->entry[i].type));
            cprintf_field(" Tag: ", "%#x ", tabl->entry[i].tag);
            cprintf(RED, "(%s)", stmp_get_logical_drive_tag_string(tabl->entry[i].tag));
            unsigned long long size = tabl->entry[i].size;
        ...
        for(int i = 0; i < tabl->header.count; i++)
        {
            uint8_t drive = tabl->entry[i].drive_no;
            cprintf(BLUE, "Drive ");
            cprintf(YELLOW, "%02x\n", drive);
